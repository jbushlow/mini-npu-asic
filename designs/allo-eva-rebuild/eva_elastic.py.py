...
def get_eva_top_elastic(Ty: AlloType = float16):
    ...
    @df.region()
    def top(
        # All inputs precede all outputs. Keep this order aligned with the
        # first-use order established by the driver/collector kernels below.
        in_w: Ty[M, LANELEN], iv_w: int32[M, LANELEN],
        in_e: Ty[M, LANELEN], iv_e: int32[M, LANELEN],
        in_n: Ty[N, LANELEN], iv_n: int32[N, LANELEN],
        in_s: Ty[N, LANELEN], iv_s: int32[N, LANELEN],
...
# selected architecture explicit here so the emitted metadata and workload use
# exactly the same top function and scalar type.
top = get_eva_top_elastic(float16)
...
def testbench_workload():
    """Return a deterministic two-hop router-transit smoke transaction.

    Packets injected from the west use destination ID 15, which cannot match
    either column of the 2x2 mesh. They therefore traverse both EVA nodes and
    must emerge from the east edge in order. Systolic valids are zero so this
    first ASIC test isolates router transport and nonblocking stream behavior.
    """
    fp_zero = np.zeros((M, LANELEN), dtype=np.float16)
    int_zero_rows = np.zeros((M, LANELEN), dtype=np.int32)
    int_zero_cols = np.zeros((N, LANELEN), dtype=np.int32)
    sentinel_fp_rows = np.full((M, LANELEN), np.float16(-7.0), dtype=np.float16)
    sentinel_fp_cols = np.full((N, LANELEN), np.float16(-7.0), dtype=np.float16)
    sentinel_int_rows = np.full((M, LANELEN), -7, dtype=np.int32)
    sentinel_int_cols = np.full((N, LANELEN), -7, dtype=np.int32)
    ...
    destination_id = 15
    request = 1
    west_packets = np.zeros((M, LANELEN), dtype=np.int32)
    for row in range(M):
        for lane in range(LANELEN):
            data = (row * LANELEN + lane + 1) & data_mask
            packet = (
                data
                | (address << 16)
                | (mode << 20)
            ...
            west_packets[row, lane] = np.int32(packet)

    arguments = {
        "in_w": fp_zero.copy(),
        "in_e": fp_zero.copy(),
        "in_n": np.zeros((N, LANELEN), dtype=np.float16),
        "in_s": np.zeros((N, LANELEN), dtype=np.float16),
        "out_w": sentinel_fp_rows.copy(),
        "out_e": sentinel_fp_rows.copy(),
        "out_n": sentinel_fp_cols.copy(),
        "out_s": sentinel_fp_cols.copy(),
        "rin_w": west_packets,
        "rin_e": int_zero_rows.copy(),
        "rin_n": int_zero_cols.copy(),
        "rin_s": int_zero_cols.copy(),
        "rout_w": sentinel_int_rows.copy(),
        "rout_e": sentinel_int_rows.copy(),
        "rout_n": sentinel_int_cols.copy(),
        "rout_s": sentinel_int_cols.copy(),
        "iv_w": int_zero_rows.copy(),
        "iv_e": int_zero_rows.copy(),
        "iv_n": int_zero_cols.copy(),
        "iv_s": int_zero_cols.copy(),
    }
    expected = {
        "out_w": sentinel_fp_rows.copy(),
        "out_e": sentinel_fp_rows.copy(),
    ...
    return {
        ...
        "calls": [{
            "name": "west_to_east_router_transit",
            "reset_before": True,
            "arguments": arguments,
            "expected": expected,
            "comparison": {
                name: {"mode": "bit_exact"} for name in expected
            },
        }],
