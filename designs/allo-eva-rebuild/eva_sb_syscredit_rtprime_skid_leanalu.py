...
def get_eva_top(Ty: AlloType = float16):
    ...
    def top(
        ...
        def drv_w(din: Ty[M, LANELEN], vd: int32[M, LANELEN]):
            ...
            dcred: int32[M] = 0
            rp: int32[M] = 0               # fetch ptr (leads pops by wcnt)
            wcnt: int32[M] = 0
            win_d: Ty[M, WSKID] = 0        # prefetched data
            win_f: int32[M, WSKID] = 0     # prefetched vd flag
            win_s: int32[M, WSKID] = 0     # slot index (eligibility)
            zw: SYS_W = 0                    # Option-A neutral prime
            for _pt in range(PRIME_TOKENS - 1):
                with allo.meta_for(0, M) as r:
                    sys_e[r, 0].put(zw)
            for _pf in range(PREFILL):       # PREFILL: head is never fetched
                with allo.meta_for(0, M) as r:  # in the iteration that pops it
                    if rp[r] < LANELEN:
        ...
        def drv_n(din: Ty[N, LANELEN], vd: int32[N, LANELEN]):
            ...
            # read (>=1 iteration early), which is invisible on the wire.
            dcred: int32[N] = 0
            rp: int32[N] = 0               # fetch ptr (leads pops by wcnt)
            wcnt: int32[N] = 0
            win_d: Ty[N, WSKID] = 0        # prefetched data
            win_f: int32[N, WSKID] = 0     # prefetched vd flag
            win_s: int32[N, WSKID] = 0     # slot index (eligibility)
            zw: SYS_W = 0                    # Option-A neutral prime
            for _pt in range(PRIME_TOKENS - 1):
                with allo.meta_for(0, N) as r:
                    sys_s[0, r].put(zw)
            for _pf in range(PREFILL):       # PREFILL: head is never fetched
                with allo.meta_for(0, N) as r:  # in the iteration that pops it
                    if rp[r] < LANELEN:
                        win_d[r, wcnt[r]] = din[r, rp[r]]
                        win_f[r, wcnt[r]] = vd[r, rp[r]]
                        win_s[r, wcnt[r]] = rp[r]
        ...
        def rdrv_w(rdin: int32[M, LANELEN]):
            ...
            win_p: Pkt[M, WSKID] = 0       # prefetched packets
        ...
        def rdrv_n(rdin: int32[N, LANELEN]):
            ...
            win_p: Pkt[N, WSKID] = 0       # prefetched packets
            zp: Pkt = 0                      # Option-A neutral prime
            for _pt in range(PRIME_TOKENS - 1):
                with allo.meta_for(0, N) as r:
                    rtr_s[0, r].put(zp)
